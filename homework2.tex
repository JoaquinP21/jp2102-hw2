...
nanaBecause alpha-beta pruning is highly dependent on the order in which nodes are explored, different pruning occures. When good moves are evaluated earlier, tighter bounds will be established sooner, allowing for more branches to be pruned.
\section*{Problem 8}
Which of the following are consistent?

We know a heuristic is consistent if $h(n) <= c(n,n') + h(n')$ for every edge. Given that h1 and h2 are consistent:
...
min{h1(n), h2(n)} <= c + min{h1(n'), h2(n')}

Therefore, a is consistent.

b) h(n) = w h1(n) + (1 - w) h2(n)

Using consistency of h1 and h2:
...
We know a heuristic is admissible if h(n) <= h*(n) for every node, where h*(n) is the true optimal cost from n to the goal. Given that h1 and h2 are admissible:
...
b) h(n) = w h1(n) + (1 - w) h2(n), where 0 <= w <= 1

Using admissibility of h1 and h2:
